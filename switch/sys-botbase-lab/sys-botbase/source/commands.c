...
void attach()
{
    ...
    if (debugHold && debughandle != 0)
        return; // lab: keep the paused session's handle
...
void detach() {
    if (debugHold)
        return; // lab: `resume` closes the held handle
    if (debughandle != 0)
    {
        svcCloseHandle(debughandle);
        debughandle = 0;
...
// ---- sys-botbase-lab additions ------------------------------------------
// All replies are one line unless stated. Addresses are printed as bare hex.

// Print every mapped region as "addr size type perm " tuples on one line.
void queryMemoryAll(void)
{
    attach();
    if (debughandle == 0)
    {
    ...
    MemoryInfo info;
    u32 pageinfo;
    while (addr < 0x8000000000ULL)
    {
        Result rc = svcQueryDebugProcessMemory(&info, &pageinfo, debughandle, addr);
        if (R_FAILED(rc))
        {
            if (debugResultCodes)
                printf("svcQueryDebugProcessMemory: %d\n", rc);
            break;
        }
        u32 type = info.type & 0xFF;
        if (type != MemType_Unmapped)
            printf("%lX %lX %X %X ", info.addr, info.size, type, info.perm);
...
// Binary read: 8-byte little-endian byte count, then the bytes. Count is 0
// when the range is not fully readable. Capped at 2 MiB per call.
void peekRaw(u64 offset, u64 size)
{
    u64 zero = 0;
    const u64 cap = 2 * 1024 * 1024;
    u8* out = (size > 0 && size <= cap) ? malloc(size) : NULL;
    if (out == NULL)
    {
        fflush(stdout);
    ...
    else
    {
        ...
        while (sent < size)
        {
            ssize_t n = write(STDOUT_FILENO, out + sent, size - sent);
            if (n <= 0)
                break;
            sent += n;
        }
...
// On-device exact search. Prints matching addresses (hex, space separated)
// at `width`-aligned offsets. Appends "!" if a chunk could not be read and
// "+" if the hit cap was reached.
void searchValue(u64 width, u64 value, u64* starts, u64* sizes, u64 count)
{
    const u64 CHUNK = 1024 * 1024;
    const u64 MAX_HITS = 200000;
    ...
    for (u64 r = 0; r < count && hits < MAX_HITS; r++)
    {
        u64 addr = starts[r];
        ...
        while (addr < end && hits < MAX_HITS)
        {
            ...
            for (u64 i = 0; i + width <= n; i += width)
            {
                ...
                switch (width)
                {
                    case 1: v = buf[i]; break;
                    case 2: v = *(u16*)(buf + i); break;
                    case 4: v = *(u32*)(buf + i); break;
                    default: v = *(u64*)(buf + i); break;
                }
                if (v == value)
                {
                    printf("%lX ", addr + i);
...
// Freeze every thread of the game until `resume` (or client disconnect).
...
Result resumeProcess(void)
{
    ...
    while (R_SUCCEEDED(svcGetDebugEvent(&event, debughandle)))
        ;
    Result rc = svcContinueDebugEvent(debughandle, 4 | 2 | 1, NULL, 0);
    debugHold = false;
    detach();
    return rc;
}
